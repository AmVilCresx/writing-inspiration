  ...
  return (
    <div style={{ marginTop: 48 }}>
      ...
      <form onSubmit={handleAdd} style={{ display: "flex", gap: 12, marginBottom: 20 }}>
        <input value={tagName} onChange={(e) => setTagName(e.target.value)} placeholder="新标签名" maxLength={20}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
        {tags.map((tag) => (
          <span key={tag.id} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 14px", background: "rgba(255,255,255,0.5)", borderRadius: 16, fontSize: 13, border: "1px solid rgba(255,255,255,0.4)", color: "var(--fg)" }}>
            {tag.name}
            <button onClick={() => handleDelete(tag.id, tag.name)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--fg-muted)", fontSize: 14, padding: 0, lineHeight: 1 }}>×</button>
          </span>
        ))}
      </div>
    </div>
